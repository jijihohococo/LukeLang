...
| Networked DB (Postgres / libpq) | ✅ | `std/pg` — TLS pool + async pipelined executor — [`NETWORK_DB_ROADMAP.md`](./NETWORK_DB_ROADMAP.md) |
...
| C10K concurrency (pool / evented I/O) | ✅ | SO_REUSEPORT multi-loop + per-loop pools; keep-alive 100k; idle timeouts; graceful SIGTERM — see [`DEPLOY.md`](./DEPLOY.md) |
